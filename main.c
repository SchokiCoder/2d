...
void btn_settings_click(void *ptr)
{
	...
	data->txt_gfx_window_w->text.len =
	    SM_strlen(data->txt_gfx_window_w->text.str) - 1;
	...
	data->txt_gfx_window_h->text.len =
	    SM_strlen(data->txt_gfx_window_h->text.str) - 1;
	...
	data->txt_gfx_window_fullscreen->text.len =
	    SM_strlen(data->txt_gfx_window_fullscreen->text.str) - 1;
...
int main()
{
	...
	SGUI_Button_new(&btn_start_game_close, &mnu_start_game, font,
			THEME_SUB.button);
	...
	SGUI_Button_new(&btn_settings_close, &mnu_settings, font,
			THEME_SUB.button);
	...
	SGUI_Label_new(&lbl_gfx_window_fullscreen, &mnu_settings, font,
		       THEME_SUB.label);
	SGUI_Entry_new(&txt_gfx_window_fullscreen, &mnu_settings, font,
		       THEME_SUB.entry);

	mnu_license = SGUI_Menu_new(renderer, THEME_SUB.menu);
	SGUI_Button_new(&btn_license_close, &mnu_license, font,
			THEME_SUB.button);
	...
	SM_String_copy_cstr(&lbl_source1.text,
